...
SELECT DO.ID AS IDDICIPLINA_OFERTADA, COUNT(SM.ID_DISCIPLINA_OFERTADA) as TOTAL_ALUNOS
FROM DISCIPLINA_OFERTADA DO
LEFT JOIN SOLICITACAO_MATRICULA SM ON DO.ID = SM.ID_DISCIPLINA_OFERTADA
...
SELECT	A.ID AS IDALUNO, DO.ID AS IDDISCIPLINA_OFERTADA, AVG(ISNULL(E.NOTA, 0)) AS MEDIA
FROM ALUNO A
...
INNER JOIN ATIVIDADE_VINCULADA AV ON DO.ID = AV.ID_DISCIPLINA_OFERTADA
LEFT JOIN ENTREGA E ON A.ID = E.ID_ALUNO
GROUP BY E.NOTA, A.ID, DO.ID ORDER BY NOTA DESC
